        ...
        for i in range(self.max_interactions): # TODO: 2 - Set loop to iterate up to the maximum number of interactions:
           ...
           # print(f"Worker Agent Response:\n{response_from_worker}")
            ...
            if evaluation.lower().startswith("yes"):
                print(f"✅ Final solution accepted after {i+1} steps")
                break
            else:
                #print(" Step 4: Generate instructions to correct the response")
                instruction_prompt = (
                    f"Provide exact instructions to fix an answer based on these reasons why it is incorrect: {evaluation}"
                )
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages=[
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        steps = [s for s in response_text.split("\n")   if s.strip() and s.strip()[0].isdigit()]
